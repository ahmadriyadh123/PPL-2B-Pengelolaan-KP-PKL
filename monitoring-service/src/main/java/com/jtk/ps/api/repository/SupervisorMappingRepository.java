...
public interface SupervisorMappingRepository extends JpaRepository<SupervisorMapping, Integer> {
    ...
    @Query(value = "select * from supervisor_mapping where id in (select min(id) from supervisor_mapping where prodi_id = :prodi group by company_id)",nativeQuery = true)
    List<SupervisorMapping> findByProdiGroupByCompanyId(@Param("prodi") int prodi);
    @Query(value = "select * from supervisor_mapping where id in (select min(id) from supervisor_mapping where lecturer_id = :lecturer group by company_id)",nativeQuery = true)
