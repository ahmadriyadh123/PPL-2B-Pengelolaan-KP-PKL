...
const PengelolaanKegiatan = () => {
  ...
  const refreshData = (index) => {
    axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/timeline`)
      .then((result) => {
        setTimeline(result.data.data)
        axios
          .get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/form-submit-time`)
          .then((result) => {
  ...
  function handleChange(value) {
    setChoose((pre) => {
      return {
        ...pre,
        id_timeline: value,
        description: timeline.find((item) => item.id === value).description,
      }
    })
  }
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        if (
          ...
          return `${start_date.toLocaleDateString('en-GB', {
            day: 'numeric',
  ...
  const showModalEditFormulir = (record) => {
    ...
    setChoose((pre) => {
      return {
        ...pre,
        description: timeline.find((item) => item.id === record.id_timeline).description,
      }
    })
  }
  ...
  const handleOkCreate = async (index) => {
    enterLoading(index)
    await axios
      .post(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/timeline/create`, {
        name: name,
  ...
  const handleOkEditTimeline = async (index) => {
    ...
    await axios
      .put(
        `${process.env.REACT_APP_API_GATEWAY_URL}management-content/timeline/update/${choose.id}`,
        {
          name: choose.name,
  ...
  const handleOkEditFormulir = async (index) => {
    ...
    await axios
      .put(
        `${process.env.REACT_APP_API_GATEWAY_URL}management-content/form-submit-time/update/${choose.id}`,
        {
  ...
  const handleOkDelete = async (id, index) => {
    ...
    await axios
      .delete(
        `${process.env.REACT_APP_API_GATEWAY_URL}management-content/timeline/delete/${id}`,
        {},
      )
      .then((response) => {
        refreshData(index)
    ...
    <>
      <div style={{ paddingBottom: '20px' }}>
        <Alert
          ...
          description={
            <>
              <>
                1. Jika kegiatan tidak dapat dihapus maka kegiatan tersebut merupakan kegiatan KP
                dan PKL.
              </>
              <br />
              <>
                2. Jika formulir tidak dapat diubah maka formulir tersebut merupakan kegiatan KP dan
                PKL.
              </>
              <br />
              <>
                3. Suatu formulir tidak dapat berada pada satu kegiatan yang sama dengan formulir
                lainnya.
              </>
            </>
          }
            ...
            <CCol sm={12}>
              <Tabs type="card">
                <TabPane tab="Kegiatan" key="1">
                  ...
                  <h6>
                    Tabel data kegiatan {localStorage.getItem('id_prodi') === '0' ? 'KP' : 'PKL'}
                  </h6>
                  <Table
                    scroll={{ x: 'max-content' }}
                    columns={columnsTimeline}
                    dataSource={getData(timeline).sort((a, b) =>
                ...
                <TabPane tab="Waktu Pengumpulan Formulir" key="2">
                  <h6>
                    Tabel data formulir {localStorage.getItem('id_prodi') === '0' ? 'KP' : 'PKL'}
                  </h6>
                  <Table
                    scroll={{ x: 'max-content' }}
                    columns={columnsFormulir}
                    dataSource={formulir}
                    pagination={false}
                    rowKey="id"
                    bordered
      ...
      >
        ...
        >
          ...
          <b>
            Tanggal Awal dan Akhir<span style={{ color: 'red' }}> *</span>
          </b>
          <Form.Item
            name="tanggal"
          ...
          >
            <RangePicker
              ...
              style={{ width: '100%' }}
              value={
                date.start_date !== '' ? [moment(date.start_date), moment(date.end_date)] : null
              }
            />
          </Form.Item>
          ...
          <div style={{ paddingTop: '24px' }}>
            ...
            >
              <Input onChange={(e) => setKeterangan(e.target.value)} />
            </Form.Item>
          </div>
        </Form>
      </Modal>
      ...
      >
        ...
        >
          ...
          <RangePicker
            ...
            style={{ width: '100%' }}
            value={
              choose.start_date !== '' ? [moment(choose.start_date), moment(choose.end_date)] : null
            }
          />
      ...
      <Modal
        title="Ubah Data Waktu Pengumpulan Formulir"
      ...
      >
        ...
        >
          <b>
            Nama Formulir<span style={{ color: 'red' }}> *</span>
          </b>
          <Form.Item
            name="name"
            rules={[{ required: true, message: 'Nama formulir tidak boleh kosong!' }]}
          >
            <Input disabled value={choose.name} />
          </Form.Item>
          ...
          <br></br>
          <Form.Item name="kegiatanForm">
            <Select value={choose.description} style={{ width: '100%' }} onChange={handleChange}>
              {timeline
                .filter((item) => item.prodi_id === parseInt(localStorage.getItem('id_prodi')))
                .map((item, i) => (
                  <Option
                    key={i}
                    value={item.id}
                    disabled={formulir.filter((i) => i.id_timeline === item.id).length > 0}
                  >
                    {item.description}
                  </Option>
                ))}
          ...
          <b>
            Hari sebelum waktu tutup formulir<span style={{ color: 'red' }}> *</span>
          </b>
          <Form.Item
            name="nameNotification"
            rules={[{ required: true, message: 'Nama formulir tidak boleh kosong!' }]}
