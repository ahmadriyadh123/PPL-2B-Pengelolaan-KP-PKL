...
import { faPencil, faLock, faTrashCan } from '@fortawesome/free-solid-svg-icons'
import {
  Tabs,
  Table,
  Button,
...
const PengelolaanAkun = () => {
  ...
  useEffect(() => {
    ...
    async function getDataAccount() {
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}account/get-all`, {
          signal: controller.signal,
        })
        .then((result) => {
          filterDataAccount(result.data.data)
          setIsLoading(false)
          console.log('data perusahaan => ', result.data.data.company)
        })
  ...
  const handleOkCreate = async (index) => {
    enterLoading(index)
    await axios
      .post(`${process.env.REACT_APP_API_GATEWAY_URL}account/create`, {
        username: username,
        password: '1234',
        id_role: role,
        name: name,
  ...
  const handleOkEdit = async (index) => {
    enterLoading(index)
    await axios
      .post(`${process.env.REACT_APP_API_GATEWAY_URL}account/update`, {
        // username: username,
  ...
  const handleOkLock = async (index) => {
    enterLoading(index)
    await axios
      .post(`${process.env.REACT_APP_API_GATEWAY_URL}account/committee-change-password`, {
        id_account: choose.id_account,
  ...
  const handleOkDelete = async (id, index) => {
    enterLoading(index)
    await axios
      .post(`${process.env.REACT_APP_API_GATEWAY_URL}account/delete`, {
        id_account: id,
      })
      .then((response) => {
        refreshData(index)
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Form.Item name="name" rules={[{ required: true, message: 'Nama tidak boleh kosong!' }]}>
          ...
          <Form.Item name="role" rules={[{ required: true, message: 'Role harus dipilih!' }]}>
            <Radio.Group>
              <Radio value="3" onClick={() => setRole(3)}>
                Ketua Prodi
              </Radio>
              <Radio value="0" onClick={() => setRole(0)}>
                Panitia
              </Radio>
            </Radio.Group>
          </Form.Item>
      ...
      >
        <Form form={form2} name="basic" onFinish={() => handleOkLock(0)} wrapperCol={{ span: 24 }}>
          <b>
