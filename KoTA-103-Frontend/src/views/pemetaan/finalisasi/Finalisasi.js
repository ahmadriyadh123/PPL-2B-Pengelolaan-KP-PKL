...
const Finalisasi = () => {
  ...
  useEffect(() => {
    ...
    async function getDataMapping() {
      ...
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/final`, {
        ...
        .then((result1) => {
          ...
          setDataP(data)
          axios
            .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/get-all?type=dropdown`, {
              signal: controller.signal,
            })
            .then((result2) => {
              setDataPerusahaan(result2.data.data)
              axios
                .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/get-all?type=dropdown`, {
                  signal: controller.signal,
                })
                .then((result3) => {
  ...
  const filterPerusahaan = (data) => {
    ...
    let jumlah
    data.forEach((item, i) => {
      jumlah = dataPemetaan.final_mapping.filter(
        (itemPemetaan) => itemPemetaan.company.name === item.name,
      ).length
  ...
  const addPemetaan = async (index) => {
    ...
    if (dataPemetaan.is_final === 0) {
      await axios
        .post(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/final/create`, {
  ...
  const deletePemetaan = async (data, index) => {
    ...
    if (dataPemetaan.is_final === 0) {
      await axios
        .delete(
          `${process.env.REACT_APP_API_GATEWAY_URL}mapping/final/delete/${data.participant.id}`,
        )
        .then((response) => {
          refreshData(index)
          notification.success({
  ...
  const handleOkCreate = async (status, index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/final/submit`)
      .then((response) => {
        refreshData(index)
        notification.success({
          message:
            status === 0 ? 'Pemetaan berhasil difinalisasi' : 'Finalisasi berhasil dibatalkan',
        })
        form.resetFields()
      })
      .catch((error) => {
  ...
  const finalisasi = (status, index) => {
    if (dataPemetaan.final_mapping.length === dataMahasiswa.length) {
      if (status === 0) {
        ...
        Modal.confirm({
          title:
            'Setelah dibatalkan, peserta tidak dapat melihat hasil pemetaan yang telah dilakukan!',
  ...
  const publish = (index) => {
    if (dataPemetaan.is_final === 1) {
      Modal.confirm({
        title:
          'Setelah diumumkan, perusahaan dapat melihat mahasiswa yang dialokasikan pada perusahaannya!',
  ...
  const handleOkPublish = async (index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/final/publish`)
      .then((response) => {
        refreshData(index)
        notification.success({
      ...
      <div style={{ padding: 8 }}>
        ...
        <Space>
          ...
          <Button
            ...
            onClick={() => handleReset(clearFilters, '', confirm, dataIndex, `reset`)}
            size="small"
            style={{ width: 90 }}
          >
            Reset
  ...
  const handleSearch = (selectedKeys, confirm, dataIndex, index) => {
    ...
    axios.defaults.withCredentials = true
    axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/final`).then((result1) => {
      result1.data.data.final_mapping.map((item) => {
        return item.participant.map((item2) => {
          return data.final_mapping.push({
            id: item.company.company_name + item2.name,
      ...
      if (selectedKeys[0] === undefined) {
        ...
        setDataPemetaan({
          final_mapping: data.final_mapping.filter((item) =>
            item.participant.name.includes(selectedKeys[0]),
          ),
    ...
    let kolom = [
      {
        ...
        render(_, row) {
          return (
            <>
              <Row align="middle">
                <Col>{row.company.name}</Col>
                <Col style={{ paddingLeft: '10px' }}>
                  {dataP &&
                  dataP.final_mapping.filter((item) => item.company.name === row.company.name)
                    .length ===
                    dataPerusahaan.find((item) => item.name === row.company.name).quota ? (
        ...
        filters:
          dataPemetaan &&
          _.uniqWith(
            filterData(dataPemetaan.final_mapping)((i) => i.company.name),
            _.isEqual,
          ),
      ...
      },
      {
        ...
        render: (_, record) => {
          return dataPerusahaan.find((item) => item.name === record.company.name).quota
        },
      },
      {
        ...
        render: (item, record) => <>{record.participant.name}</>,
      },
    ]
      ...
      kolom.push({
        ...
        render: (value, item, index) => (
          <>
            ...
            >
              {loadings[`delete-${index + 2}`] === false ||
              loadings[`delete-${index + 2}`] === undefined ? (
  ...
  const changeCompany = async (value) => {
    ...
    let no = 0
    await axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/get-participant-by-company/${value}`)
      .then((result) => {
        result.data.data.map((item) => {
          ...
          return data.push({
            ...
            name: `${item.name} ${
              item.value !== 0
                ? `(${(item.value * 100).toFixed(2)}%${no === 1 ? ' direkomendasikan' : ''})`
                : ''
            }`,
          })
        })
      ...
      {getNeedKuota() > 0 && (
        <div style={{ paddingBottom: '20px' }}>
          <Alert
            ...
            description={
              <>
                Jumlah kuota seluruh perusahaan belum memenuhi jumlah seluruh mahasiswa. Dibutuhkan{' '}
                <b>{getNeedKuota()} kuota</b> untuk memenuhi jumlah seluruh mahasiswa
              </>
            }
            type="error"
            showIcon
        ...
        <div style={{ paddingBottom: '20px' }}>
          <Alert
            ...
            description={
              <>
                <>
                  1. Finalisasi pemetaan terakhir dilakukan pada tanggal{' '}
                  {getDate(dataPemetaan.final_date)}.
                </>
                <br />
                {dataPemetaan.is_publish === 1 && (
                  <>
                    2. Umumkan ke perusahaan terakhir dilakukan pada tanggal{' '}
                    {getDate(dataPemetaan.publish_date)}.
                  </>
                )}
              </>
            }
      ...
      <Row justify="end">
        <Col style={{ textAlign: 'center', paddingBottom: '15px', paddingRight: '15px' }}>
          {dataPemetaan.is_final === 0 ? (
            ...
            <Button
              ...
              loading={loadings[5]}
              style={
                dataPemetaan.is_publish === 0
                  ? { color: 'black', background: '#FCEE21', borderColor: '#FCEE21' }
                  : { color: 'black', background: '#BBB11A', borderColor: '#BBB11A' }
              }
              onClick={() => publish(5)}
            >
              Umumkan ke perusahaan
        ...
        <Col style={{ textAlign: 'center', paddingBottom: '15px' }}>
          <Button
            ...
            disabled={dataPemetaan.is_publish === 1 ? true : false}
            style={
              dataPemetaan && dataPemetaan.is_final === 0
                ? { backgroundColor: '#00AF8F', borderColor: '#00AF8F' }
                : dataPemetaan.is_publish === 1
                ? { color: 'white', background: '#900427', borderColor: '#900427' }
                : { color: 'white', background: '#CC0033', borderColor: '#CC0033' }
            }
        ...
        <CCardBody>
            ...
            <CRow>
              <CCol sm={5} style={{ padding: '10px' }}>
                ...
                >
                  <Select
                    disabled={dataPemetaan.is_final === 1}
                    onSelect={(value) => changeCompany(value)}
                  >
                    {filterPerusahaan(dataPerusahaan).map((item, i) => (
                      <Select.Option key={i} value={item.id}>
                        <>
                          {item.name}
                          {item.pemetaan === 0 ? (
                            <span style={{ color: 'red' }}> *belum dialokasikan sama sekali</span>
                          ) : (
                            ` (tersisa ${item.quota - item.pemetaan} kuota)`
                          )}
                        </>
                      </Select.Option>
                    ))}
                ...
                <CCol sm={5} style={{ padding: '10px' }}>
                  ...
                  >
                    <Select
                      ...
                      notFoundContent={
                        !(dataRankPeserta && dataRankPeserta.length > 0) ? (
                          <Spin size="small" />
                        ) : (
                          ''
                        )
                      ...
                      disabled={
                        dataPemetaan.is_final === 1
                          ? true
                          : dataRankPeserta && isCompanySelected
                          ? false
                          : true
                      }
                    >
                      {dataRankPeserta &&
                        dataRankPeserta.map((item, i) => (
                          <Select.Option key={i} value={item.id}>
                            {item.name}
                          </Select.Option>
                        ))}
                ...
                <CCol sm={5} style={{ padding: '10px' }}>
                  ...
                  >
                    <Tooltip placement="top" title="Semua mahasiswa sudah teralokasikan">
                      <Select onSelect={(value) => setMahasiswa(value)} disabled={true}>
                        {dataRankPeserta &&
                          dataRankPeserta.map((item, i) => (
                            <Select.Option key={i} value={item.id}>
                              {item.name}
          ...
          <CRow>
            ...
            <Table
              ...
              rowKey="id"
              columns={dataPemetaan && getColumns()}
              dataSource={createNewArr(dataPemetaan.final_mapping)}
              pagination={false}
              bordered
            />
          </CRow>
