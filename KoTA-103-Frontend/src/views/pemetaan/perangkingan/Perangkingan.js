...
const Perangkingan = () => {
  ...
  const generatePerangkingan = async (index) => {
    enterLoading(index)
    await axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/generate-rank`)
  ...
  const eksporPerangkingan = async (index) => {
    ...
    axios.defaults.withCredentials = true
    await axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/export-mapping`, {
        responseType: 'blob',
      })
      .then(function (response) {
        notification.success({
  ...
  useEffect(() => {
    ...
    const getRangking = async () => {
      ...
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/get-rank`, {
  ...
  const columns = [
    {
      ...
      filterMode: 'tree',
      filters: _.uniqWith(
        filterData(data.rangking === undefined ? [] : data.rangking)((i) => i.company_name),
        _.isEqual,
      ),
    ...
    },
    {
      ...
      width: '10%',
      align: 'center',
      render: (item, record) => <>{record.result.toFixed(4)}</>,
    },
    {
      title: 'Rangking',
      dataIndex: 'rangking',
    ...
    <>
      {data.rangking.length !== 0 && (
        <div style={{ paddingBottom: '20px' }}>
          <Alert
            message="Catatan"
            description={`Generate perangkingan terakhir dilakukan pada tanggal ${getDate(
              data.date,
            )}.`}
            type="info"
            showIcon
        ...
        <CCardBody>
          <CRow>
            <CCol sm={6} style={{ padding: '10px' }}>
              <Button
                loading={loadings[1]}
                onClick={() => generatePerangkingan(1)}
                type="primary"
                htmlType="submit"
                className="px-3"
                id="generate"
            ...
            <Table
              rowKey={'id'}
              columns={columns}
              dataSource={createNewArr(data.rangking === undefined ? [] : data.rangking)}
              bordered
              pagination={false}
