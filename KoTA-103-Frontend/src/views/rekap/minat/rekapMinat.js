...
const RekapMinat = () => {
  ...
  useEffect(() => {
    ...
    const getRecap = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/company-selection/recap`, {
    ...
    const getMinat = async () => {
      ...
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/company-selection`, {
          signal: controller.signal,
        })
        .then(function (response) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <>
          <Row align="middle">
            <Col>{record.participant_name}</Col>
            <Col style={{ paddingLeft: '10px' }}>
              {record.status ? (
                <Tooltip
                  placement="right"
                  title="Sudah mengisi formulir pilihan prioritas perusahaan"
                >
                  <Button
                    type="primary"
                    shape="circle"
                ...
                <Tooltip
                  placement="right"
                  title="Belum mengisi formulir pilihan prioritas perusahaan"
                >
                  <Button
                    type="primary"
                    shape="circle"
  ...
  const getPersentase = (total) => {
    return localStorage.getItem('id_role') === '3'
      ? localStorage.getItem('id_prodi') === '0'
        ? ((total / 61) * 100).toFixed(2)
        : ((total / 27) * 100).toFixed(2)
      : ((total / data.company_selection.length) * 100).toFixed(2)
  }
                        ...
                        <Row justify="space-around" align="middle">
                          ...
                          <Col span={18} style={{ paddingTop: '10px' }}>
                            ...
                            <h5 style={{ color: '#CC0033' }}>
                              {data.total_not_selected} Mahasiswa
                            </h5>
                          </Col>
                        </Row>
                      </CCardBody>
                    ...
                    <CRow>
                      <CCol sm={12}>
                        ...
                        <Table
                          ...
                          columns={columns}
                          dataSource={
                            data && status.check === false && status.x === false
                        ...
                        <CCol sm={12}>
                          ...
                          <Table
                            ...
                            dataSource={rekap
                              ...
                              .concat(rekap.filter((item) => item.priority === 0))
                              .sort((a, b) => {
