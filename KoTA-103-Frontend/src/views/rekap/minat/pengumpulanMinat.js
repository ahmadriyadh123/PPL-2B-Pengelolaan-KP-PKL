...
const PengumpulanMinat = () => {
  ...
  const refreshData = (index) => {
    axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/company-selection/card`)
  ...
  const handleMark = async (index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}participant/company-selection/mark-as-done`, {})
      .then((response) => {
        refreshData(index)
        notification.success({
          message:
            data && data.status_submission === false
