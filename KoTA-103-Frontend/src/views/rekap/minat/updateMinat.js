...
const UpdateMinat = () => {
  ...
  useEffect(() => {
    ...
    const getDetailMinat = async () => {
      ...
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/company-selection/detail`, {
  ...
  const onFinish = async (index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}participant/company-selection/update`, {
    ...
    <>
      <CCard>
        <CCardHeader style={{ paddingLeft: '20px' }}>
          <h5>
            <b>Update Pemilihan Perusahaan</b>
          </h5>
        </CCardHeader>
                    ...
                    {company &&
                      company.map((item, i) => (
                        <Select.Option
                          key={i}
                          value={item.id}
                          disabled={chooseCompany.includes(item.id)}
                        >
                          {item.name}
                        </Select.Option>
                      ))}
