    ...
    useEffect(() => {
        if (!id) {
            ...
            const getDetail = async () => {
                ...
                axios.defaults.withCredentials = true;
                try {
                    const response = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}company/prerequisite/${id}`)
                    ...
                    if (response.data.data.region_id === 0) {
                        ...
                        const regionResponse = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/domicile/${response.data.data.region_id}`)
                        setRegion(regionResponse.data.data)
                    }

                    const typeResponse = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/competence/get-all-type`)
