...
const UpdatePrerequisite = () => {
  ...
  const refreshData = (activeKey = '1') => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_GATEWAY_URL}management-content/competence/get-all/${activeKey}`,
      )
      .then((result) => setKompetensi(result.data.data))
    axios
      .get(
        `${process.env.REACT_APP_API_GATEWAY_URL}management-content/jobscope/get-all/${activeKey}`,
      )
      .then((result) => {
  ...
  const handleOkCreate = async (data, index) => {
    ...
    if (kompetensi === 'Cakupan Pekerjaan') {
      await axios
        .post(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/jobscope/create`, {
          name: name,
          prodi_id: prodi,
        })
        .then((response) => {
      ...
      await axios
        .post(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/competence/create`, {
          name: name,
      ...
      const getDetail = async () => {
        ...
        try {
          const response = await axios.get(
            `${process.env.REACT_APP_API_GATEWAY_URL}company/prerequisite/${id}`,
            {
          ...
          if (response.data.data.project) {
            if (response.data.data.project.split('|').length !== 0) {
              ...
              response.data.data.project.split('**')[2] === 'd3'
                ? kp.push({
          ...
          if (response.data.data.region_id === 0) {
            ...
            const regionResponse = await axios.get(
              `${process.env.REACT_APP_API_GATEWAY_URL}management-content/domicile/${response.data.data.region_id}`,
              {
                signal: controller.signal,
              },
            )
          ...
          const typeResponse = await axios.get(
            `${process.env.REACT_APP_API_GATEWAY_URL}management-content/competence/get-all-type`,
            {
              signal: controller.signal,
            },
          )
          ...
          response.data.data.description &&
            setEditorState(
              EditorState.createWithContent(
                ContentState.createFromBlockArray(htmlToDraft(response.data.data.description)),
              ),
            )
        } catch (error) {
      ...
      if (save === true) {
        ...
        if (facilitas.length > 1) {
          facilitas.map((item, index) => {
            return (fas =
              index === facilitas.length - 1
                ? fas + item.facility_name
                : fas + item.facility_name + ',')
          })
        } else {
  ...
  const onChangeKompetensi = () => {
    let data = []
    if (prodi === '0') {
      form1.getFieldValue('bahasaPemrogramanD3') &&
        form1.getFieldValue('bahasaPemrogramanD3').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('databaseD3') &&
        form1.getFieldValue('databaseD3').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('frameworkD3') &&
        form1.getFieldValue('frameworkD3').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('modellingD3') &&
        form1.getFieldValue('modellingD3').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('toolsD3') &&
        form1.getFieldValue('toolsD3').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('bahasaKomunikasiD3') &&
        form1.getFieldValue('bahasaKomunikasiD3').map((item) => {
          return item && data.push(item.id_competence)
        })
    ...
    if (prodi === '1') {
      form1.getFieldValue('bahasaPemrogramanD4') &&
        form1.getFieldValue('bahasaPemrogramanD4').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('databaseD4') &&
        form1.getFieldValue('databaseD4').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('frameworkD4') &&
        form1.getFieldValue('frameworkD4').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('modellingD4') &&
        form1.getFieldValue('modellingD4').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('toolsD4') &&
        form1.getFieldValue('toolsD4').map((item) => {
          return item && data.push(item.id_competence)
        })
      form1.getFieldValue('bahasaKomunikasiD4') &&
        form1.getFieldValue('bahasaKomunikasiD4').map((item) => {
          return item && data.push(item.id_competence)
        })
  ...
  const onChangeMinat = () => {
    ...
    if (prodi === '0') {
      if (form1.getFieldValue('cakupanPekerjaanD3')) {
        form1.getFieldValue('cakupanPekerjaanD3') !== undefined &&
          form1.getFieldValue('cakupanPekerjaanD3').map((item) => {
            return data.push(item.id_jobscope)
          })
        setChooseMinatD3(data)
      }
    ...
    if (prodi === '1') {
      if (form1.getFieldValue('cakupanPekerjaanD4')) {
        form1.getFieldValue('cakupanPekerjaanD4') !== undefined &&
          form1.getFieldValue('cakupanPekerjaanD4').map((item) => {
            return data.push(item.id_jobscope)
          })
        setChooseMinatD4(data)
      }
  ...
  const getDomisili = async (val) => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_API_GATEWAY_URL}management-content/domicile?domicile-like=${val}`,
      )
      ...
      content: (
        <CRow>
          <CCol sm={12}>
            ...
            >
              <b>
                Nama Pembimbing Industri<span style={{ color: 'red' }}> *</span>
              </b>
              <Form.Item
                name="namePembimbing"
              ...
              <Row>
                <Col span={12} style={{ paddingRight: '20px' }}>
                  <b>
                    Email Pembimbing Industri<span style={{ color: 'red' }}> *</span>
                  </b>
                  <Form.Item
                    name="emailPembimbing"
                    rules={[
                      { required: true, message: 'Email pembimbing tidak boleh kosong!' },
                ...
                <Col span={12}>
                  <b>
                    Jabatan Pembimbing Industri<span style={{ color: 'red' }}> *</span>
                  </b>
                  <Form.Item
                    name="jabatanPembimbing"
              ...
              <Row>
                <Col span={12} style={{ paddingRight: '20px' }}>
                  <b>
                    Domisili<span style={{ color: 'red' }}> *</span>
                  </b>
                  <Form.Item
                    name="domisili"
                    ...
                    <Select
                      ...
                      notFoundContent={
                        value.length >= 3 ? (
                          ...
                          <>
                            Ketikkan minimal 3 huruf untuk mencari domisili
                            <br />
                            Jika domisili tidak ada di pencarian maka pilih Lain - lain
                          </>
                        )
                      }
                ...
                <Col span={12}>
                  <b>
                    Sistem Kerja<span style={{ color: 'red' }}> *</span>
                  </b>
                  <Form.Item
                    name="sistemKerja"
                          ...
                          <Row>
                            <Col span={12} style={{ paddingRight: '20px' }}>
                              ...
                              <Form.Item
                                ...
                                fieldKey={[key, 'name']}
                                rules={[
                                  { required: true, message: 'Nama proyek tidak boleh kosong!' },
                                ]}
                              >
                                <Input />
                              </Form.Item>
                            ...
                            <Col span={12}>
                              <b>
                                Jumlah Kuota Mahasiswa<span style={{ color: 'red' }}> *</span>
                              </b>
                              <Form.Item
                                name={[name, 'kuota']}
                                fieldKey={[key, 'kuota']}
                                rules={[
                                  { required: true, message: 'Jumlah kuota tidak boleh kosong!' },
                                  {
                                    message: 'Format jumlah kuota mahasiswa hanya angka!',
                                    pattern: /^\d+$/,
                                  },
                                  {
                                    message: 'Minimal jumlah kuota adalah 1!',
                                    pattern: /\b^([A-Za-z-9]+|[1-9][0-9]*)\b/,
                                  },
                                ]}
                              >
                                <Input />
              ...
              <Form.List
                name="proyekPraktikKerjaLapangan"
                initialValue={
                  proyek.d4 &&
                  proyek.d4.map((item) => {
                    return {
                    ...
                    {fields.map(({ key, name, ...restField }) => (
                      <Row key={key}>
                        ...
                        <Col
                          span={1}
                          align="center"
                          style={{ paddingTop: '5px', maxWidth: '2%', paddingLeft: '30px' }}
                        >
                          <MinusCircleOutlined
                            onClick={() => remove(name)}
                            style={{ color: 'red' }}
            ...
            <CRow>
              <CCol sm={12}>
                ...
                >
                  ...
                  <Form.List
                    name="cakupanPekerjaanD3"
                    initialValue={
                      data &&
                      data.jobscopes_d3 &&
                      data.jobscopes_d3.length > 0 &&
                      data.jobscopes_d3.map((item) => {
                        return {
                            ...
                            <Col span={22}>
                              <Form.Item
                                ...
                                rules={[
                                  {
                                    required: true,
                                    message: 'Cakupan pekerjaan tidak boleh kosong!',
                                  },
                                ]}
                                style={{ paddingRight: '35px' }}
                              >
                                  ...
                                  dropdownRender={(menu) => (
                                    <>
                                      <Space align="center" style={{ padding: '0 8px 4px' }}>
                                        <Typography.Link
                                          onClick={() => showModalCreate('Cakupan Pekerjaan')}
                                  ...
                                  {jobscope &&
                                    jobscope.map((item, i) => (
                                      <Select.Option
                                        key={i}
                                        value={item.id}
                                        disabled={chooseMinatD3.includes(item.id)}
                                      >
                                        {item.name}
                                      </Select.Option>
                                    ))}
            ...
            <CRow>
              <CCol sm={12}>
                ...
                >
                  <b>Cakupan Pekerjaan</b>
                  <Form.List
                    name="cakupanPekerjaanD4"
                    initialValue={
                      data &&
                      data.jobscopes_d4 &&
                      data.jobscopes_d4.length > 0 &&
                      data.jobscopes_d4.map((item) => {
                        return {
                          id_jobscope: item.id_jobscope,
                        }
                                  ...
                                  {jobscope &&
                                    jobscope.map((item, i) => (
                                      <Select.Option
                                        key={i}
                                        value={item.id}
                                        disabled={chooseMinatD4.includes(item.id)}
                                      >
                                        {item.name}
                                      </Select.Option>
                                    ))}
            ...
            <CRow>
              <CCol sm={12}>
                ...
                >
                  ...
                  <Form.List
                    name="bahasaPemrogramanD3"
                    initialValue={
                      data &&
                      data.competencies_d3 &&
                      data.competencies_d3.filter((item) => item.competence_type === 1).length >
                        0 &&
                      data.competencies_d3
                        .filter((item) => item.competence_type === 1)
                        .map((item) => {
                          return {
                            ...
                            <Col span={22}>
                              <Form.Item
                                ...
                                rules={[
                                  {
                                    required: true,
                                    message: 'Bahasa pemrograman tidak boleh kosong!',
                                  },
                                ]}
                                style={{ paddingRight: '35px' }}
                              >
                                <Select
                                  ...
                                  dropdownRender={(menu) => (
                                    <>
                                      <Space align="center" style={{ padding: '0 8px 4px' }}>
                                        <Typography.Link
                                          onClick={() => showModalCreate('Bahasa Pemrograman')}
                                          style={{ whiteSpace: 'nowrap' }}
                                        >
                                    ...
                                    kompetensi
                                      ...
                                      .map((item, i) => (
                                        <Select.Option
                                          key={i}
                                          value={item.id}
                                          disabled={chooseKompetensiD3.includes(item.id)}
                                        >
                                          {item.name}
                                        </Select.Option>
                                      ))}
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d3 &&
                      data.competencies_d3.filter((item) => item.competence_type === 2).length >
                        0 &&
                      data.competencies_d3
                        .filter((item) => item.competence_type === 2)
                        .map((item) => {
                          return {
                            id_competence: item.id_competence,
                          }
                          ...
                          <Row key={key} style={{ paddingTop: '10px' }}>
                            ...
                            <Col span={22}>
                              <Form.Item
                                ...
                                fieldKey={[key, 'id_competence']}
                                rules={[
                                  { required: true, message: 'Database tidak boleh kosong!' },
                                ]}
                                style={{ paddingRight: '35px' }}
                              >
                                <Select
                                  ...
                                  dropdownRender={(menu) => (
                                    <>
                                      <Space align="center" style={{ padding: '0 8px 4px' }}>
                                        <Typography.Link
                                          onClick={() => showModalCreate('Database')}
                                          style={{ whiteSpace: 'nowrap' }}
                                        >
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d3 &&
                      data.competencies_d3.filter((item) => item.competence_type === 3).length >
                        0 &&
                      data.competencies_d3
                        .filter((item) => item.competence_type === 3)
                        .map((item) => {
                          return {
                            id_competence: item.id_competence,
                          }
                          ...
                          <Row key={key} style={{ paddingTop: '10px' }}>
                            ...
                            <Col span={22}>
                              <Form.Item
                                ...
                                fieldKey={[key, 'id_competence']}
                                rules={[
                                  { required: true, message: 'Framework tidak boleh kosong!' },
                                ]}
                                style={{ paddingRight: '35px' }}
                              >
                                <Select
                                  ...
                                  dropdownRender={(menu) => (
                                    <>
                                      <Space align="center" style={{ padding: '0 8px 4px' }}>
                                        <Typography.Link
                                          onClick={() => showModalCreate('Frameworks')}
                                          style={{ whiteSpace: 'nowrap' }}
                                        >
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d3 &&
                      data.competencies_d3.filter((item) => item.competence_type === 4).length >
                        0 &&
                      data.competencies_d3
                        .filter((item) => item.competence_type === 4)
                        .map((item) => {
                          return {
                            id_competence: item.id_competence,
                          }
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d3 &&
                      data.competencies_d3.filter((item) => item.competence_type === 5).length >
                        0 &&
                      data.competencies_d3
                        .filter((item) => item.competence_type === 5)
                        .map((item) => {
                          return {
                            id_competence: item.id_competence,
                          }
                          ...
                          <Row key={key} style={{ paddingTop: '10px' }}>
                            ...
                            <Col span={22}>
                              <Form.Item
                                ...
                                fieldKey={[key, 'id_competence']}
                                rules={[
                                  { required: true, message: 'Modelling tidak boleh kosong!' },
                                ]}
                                style={{ paddingRight: '35px' }}
                              >
                                <Select
                                  ...
                                  dropdownRender={(menu) => (
                                    <>
                                      <Space align="center" style={{ padding: '0 8px 4px' }}>
                                        <Typography.Link
                                          onClick={() => showModalCreate('Modelling Tools')}
                                          style={{ whiteSpace: 'nowrap' }}
                                        >
                  ...
                  <Form.List
                    name="bahasaKomunikasiD3"
                    initialValue={
                      data &&
                      data.competencies_d3 &&
                      data.competencies_d3.filter((item) => item.competence_type === 6).length >
                        0 &&
                      data.competencies_d3
                        .filter((item) => item.competence_type === 6)
                        .map((item) => {
                          return {
                            id_competence: item.id_competence,
                          }
                            ...
                            <Col span={22}>
                              <Form.Item
                                ...
                                rules={[
                                  {
                                    required: true,
                                    message: 'Bahasa komunikasi tidak boleh kosong!',
                                  },
                                ]}
                                style={{ paddingRight: '35px' }}
                              >
                                <Select
                                  ...
                                  dropdownRender={(menu) => (
                                    <>
                                      <Space align="center" style={{ padding: '0 8px 4px' }}>
                                        <Typography.Link
                                          onClick={() => showModalCreate('Bahasa Komunikasi')}
                                          style={{ whiteSpace: 'nowrap' }}
                                        >
            ...
            <CRow>
              <CCol sm={12}>
                ...
                >
                  <b>Bahasa Pemrograman</b>
                  <Form.List
                    name="bahasaPemrogramanD4"
                    initialValue={
                      data &&
                      data.competencies_d4 &&
                      data.competencies_d4.filter((item) => item.competence_type === 1).length >
                        0 &&
                      data.competencies_d4
                        .filter((item) => item.competence_type === 1)
                        .map((item) => {
                                    ...
                                    kompetensi
                                      ...
                                      .map((item, i) => (
                                        <Select.Option
                                          key={i}
                                          value={item.id}
                                          disabled={chooseKompetensiD4.includes(item.id)}
                                        >
                                          {item.name}
                                        </Select.Option>
                                      ))}
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d4 &&
                      data.competencies_d4.filter((item) => item.competence_type === 2).length >
                        0 &&
                      data.competencies_d4
                        .filter((item) => item.competence_type === 2)
                        .map((item) => {
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d4 &&
                      data.competencies_d4.filter((item) => item.competence_type === 3).length >
                        0 &&
                      data.competencies_d4
                        .filter((item) => item.competence_type === 3)
                        .map((item) => {
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d4 &&
                      data.competencies_d4.filter((item) => item.competence_type === 4).length >
                        0 &&
                      data.competencies_d4
                        .filter((item) => item.competence_type === 4)
                        .map((item) => {
                  ...
                  <Form.List
                    ...
                    initialValue={
                      data &&
                      data.competencies_d4 &&
                      data.competencies_d4.filter((item) => item.competence_type === 5).length >
                        0 &&
                      data.competencies_d4
                        .filter((item) => item.competence_type === 5)
                        .map((item) => {
                  ...
                  <b>Bahasa Komunikasi</b>
                  <Form.List
                    name="bahasaKomunikasiD4"
                    initialValue={
                      data &&
                      data.competencies_d4 &&
                      data.competencies_d4.filter((item) => item.competence_type === 6).length >
                        0 &&
                      data.competencies_d4
                        .filter((item) => item.competence_type === 6)
                        .map((item) => {
            ...
            >
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: `Nama ${item.name.toLowerCase()} tidak boleh kosong!`,
                  },
                ]}
              >
