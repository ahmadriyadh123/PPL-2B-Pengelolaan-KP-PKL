...
import { faPencil, faClipboard, faCheck, faEye } from '@fortawesome/free-solid-svg-icons'
...
const PrerequisitePerusahaan = () => {
  ...
  useEffect(() => {
    ...
    const getPrerequisiteCommittee = async () => {
      ...
      await axios
        .get(
          `${process.env.REACT_APP_API_GATEWAY_URL}company/prerequisite/card-by-committee?id-company=${id}`,
          {
    ...
    const getPrerequisiteCompany = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/prerequisite/card-by-company`, {
          signal: controller.signal,
        })
        .then(function (response) {
          setData(response.data.data)
    ...
    await axios
      ...
      .then((response) => {
        ...
        notification.success({
          message:
            data.status_prerequisite === false
  ...
  return isLoading ? (
    ...
    <>
      <>
        {localStorage.getItem('id_role') === '2' && data.status_submission === false && (
        ...
        {localStorage.getItem('id_role') === '0' && statusMapping === 1 && (
          <div style={{ paddingBottom: '20px' }}>
            <Alert
              message="Catatan"
          ...
          <CCardBody>
            <Row align="middle">
              ...
              {localStorage.getItem('id_role') === '2' ? (
                <>
                  <Col span={data.status_submission === true ? 10 : 19}>{data.company_name}</Col>
                  <>
                    ...
                    {data.status_submission === true && (
                      <Col span={4} style={{ textAlign: 'center' }}>
                        ...
                        >
                          <FontAwesomeIcon icon={faPencil} style={{ paddingRight: '5px' }} /> Ubah
                          Prerequisite
                        </Button>
                      </Col>
                    )}
                ...
                <>
                  <Col span={statusMapping === 0 ? 10 : 19}>{data.company_name}</Col>
                  {statusMapping === 0 && (
                    <Col span={5} style={{ textAlign: 'center' }}>
                      <Button
                        id={data.status_prerequisite === false ? 'selesai' : 'batal'}
                ...
                <>
                  <Col span={18}>{data.company_name}</Col>
                  <Col span={3} style={{ textAlign: 'center' }}>
                    <Button
                      id="detail"
                      size="small"
