...
const DetailPengajuanPerusahaan = () => {
  ...
  useEffect(() => {
    ...
    if (!id) {
      ...
      const getDetail = async () => {
        axios.defaults.withCredentials = true
        await axios
          .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/submission/detail/${id}`, {
            signal: controller.signal,
          })
          .then(function (response1) {
            setData(response1.data.data)
            axios
              .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/criteria`, {
                signal: controller.signal,
              })
  ...
  const handleOkAccept = async (index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}company/submission/accept/${id}`, {})
      .then((response) => {
        ...
        Modal.info({
          ...
          content: (
            <>
              Berikut merupakan username dan password dari akun perusahaan yang telah dibuat
              <br />
              Username : {data.company_mail}
              <br />
              Password : 1234
            </>
          ),
  ...
  const handleOkDelete = async (index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}company/submission/decline/${id}`, {})
      .then((response) => {
        notification.success({
      ...
      <CCard className="mb-4">
        <CCardHeader style={{ paddingLeft: '20px', paddingRight: '20px' }}>
          <CRow>
            <CCol>
              <h5>
                <b>Profil Perusahaan {data.company_name}</b>
              </h5>
            </CCol>
        ...
        <CCardBody style={{ paddingLeft: '20px' }}>
          <CRow>
            <CCol sm={6}>
              ...
              <CRow>
                <CCol sm={12}>{data && data.website ? data.website : 'Data belum diisi'}</CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
                  <b>Email Official Perusahaan</b>
                </CCol>
              </CRow>
              <CRow>
                <CCol sm={12}>
                  {data && data.company_mail ? data.company_mail : 'Data belum diisi'}
                </CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
              ...
              <CRow>
                <CCol sm={12}>{data && data.no_phone ? data.no_phone : 'Data belum diisi'}</CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
                  <b>Alamat</b>
              ...
              <CRow>
                <CCol sm={12}>{data && data.address ? data.address : 'Data belum diisi'}</CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
                  <b>Berdiri Tahun</b>
              ...
              <CRow>
                <CCol sm={12}>
                  {data && data.since_year ? data.since_year : 'Data belum diisi'}
                </CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
              ...
              <CRow>
                <CCol sm={12}>
                  {data && data.num_employee ? data.num_employee : 'Data belum diisi'}
                </CCol>
              </CRow>
            </CCol>
            <CCol sm={6}>
              ...
              <CRow>
                <CCol sm={12}>{data && data.cp_name ? data.cp_name : 'Data belum diisi'}</CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
                  <b>Email Narahubung</b>
                </CCol>
              </CRow>
              <CRow>
                <CCol sm={12}>{data && data.cp_mail ? data.cp_mail : 'Data belum diisi'}</CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
                  <b>Nomor Telepon Narahubung</b>
                </CCol>
              </CRow>
              <CRow>
                <CCol sm={12}>{data && data.cp_phone ? data.cp_phone : 'Data belum diisi'}</CCol>
              </CRow>
              <CRow>
                <CCol sm={12} style={{ paddingTop: '10px' }}>
                  <b>Jabatan Narahubung</b>
                </CCol>
              </CRow>
              <CRow>
                <CCol sm={12}>
                  {data && data.cp_position ? data.cp_position : 'Data belum diisi'}
                </CCol>
              </CRow>
            </CCol>
          </CRow>
          ...
          <CRow>
            <CCol sm={12} style={{ paddingTop: '10px' }}>
              <div style={{ paddingBottom: '10px' }}>
                <b>Mekanisme Penerimaan KP/PKL di Perusahaan</b>
              </div>
              <CCard>
                <CCardBody>
                  {data && data.recept_mechanism ? (
                    <div
                      className="preview"
                      dangerouslySetInnerHTML={createMarkup(data.recept_mechanism)}
                    ></div>
                  ) : (
                    'Data belum diisi'
                  )}
                          ...
                          <Col span={24} key={i}>
                            <Checkbox
                              checked={
                                data &&
                                data.criteria &&
                                data.criteria.find((data) => data === item.criteriaName)
                                  ? true
                                  : false
                              }
                              disabled
