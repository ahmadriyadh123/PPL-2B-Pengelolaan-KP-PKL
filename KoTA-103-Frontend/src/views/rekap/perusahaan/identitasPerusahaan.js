...
const IdentitasPerusahaan = () => {
  ...
  const refreshData = async (index) => {
    await axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/${id}`)
      .then(function (response) {
        setData(response.data.data)
        response.data.data.lecturer_id !== null &&
          axios
            .get(
              `${process.env.REACT_APP_API_GATEWAY_URL}account/get-committee?id=${response.data.data.lecturer_id}`,
            )
            .then(function (resp) {
              resp.data.data
                ? setData((pre) => {
  ...
  const updateStatusPerusahaan = async (index) => {
    enterLoading(index)
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}company/change-status/${id}`)
      .then(function (resp) {
        refreshData(index)
      ...
      await axios
        ...
        .then(function (response) {
          ...
          if (localStorage.getItem('id_role') !== '2') {
            ...
            axios
              .get(`${process.env.REACT_APP_API_GATEWAY_URL}mapping/get-is-final/${idMapping}`, {
                signal: controller.signal,
              })
              .then(function (response) {
        ...
        <>
          {localStorage.getItem('id_role') === '2' ? (
            <CRow>
              <CCol style={{ textAlign: 'right', paddingBottom: '15px' }}>
                ...
                >
                  <FontAwesomeIcon icon={faPencil} style={{ paddingRight: '5px' }} /> Ubah Data
                  Perusahaan
                </Button>
              </CCol>
            </CRow>
              ...
              <Row justify="end">
                <Col style={{ textAlign: 'center', paddingBottom: '15px', paddingRight: '15px' }}>
                  <Button
                    ...
                    style={
                      !data.status
                        ? { backgroundColor: '#339900', borderColor: '#339900', color: 'white' }
                        : { backgroundColor: '#CC0033', borderColor: '#CC0033', color: 'white' }
                    }
          ...
          <CCard className="mb-4">
            <CCardHeader style={{ paddingLeft: '20px' }}>
              <h5>
                <b>Profil Perusahaan {data.company_name ? data.company_name : 'Belum diisi'}</b>
              </h5>
            </CCardHeader>
            <CCardBody style={{ paddingLeft: '20px' }}>
              <CRow>
                <CCol sm={6}>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.website ? data.website : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.company_email ? data.company_email : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.telp ? data.telp : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                      <b>Alamat</b>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.address ? data.address : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                      <b>Berdiri Tahun</b>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.since_years ? data.since_years : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                      <b>Jumlah Karyawan</b>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.num_employee ? data.num_employee : 'Belum diisi'}</CCol>
                  </CRow>
                </CCol>
                <CCol sm={6}>
                  <CRow>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.lecturer_id ? data.lecturer_id : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                      <b>Nama Narahubung</b>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.cp_name ? data.cp_name : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.cp_email ? data.cp_email : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.cp_telp ? data.cp_telp : 'Belum diisi'}</CCol>
                  </CRow>
                  <CRow>
                    <CCol sm={12} style={{ paddingTop: '10px' }}>
                  ...
                  <CRow>
                    <CCol sm={12}>{data.cp_position ? data.cp_position : 'Belum diisi'}</CCol>
                  </CRow>
                </CCol>
              </CRow>
            </CCardBody>
