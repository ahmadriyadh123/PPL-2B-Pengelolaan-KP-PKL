...
const CardEvaluasiPerusahaan = () => {
  ...
  const formulirEvaluasiPerusahaan = (item) => {
    history.push({
      ...
      state: {
        id_prodi: item.participant_prodi,
        numEval: item.num_evaluation,
  ...
  useEffect(() => {
    ...
    const getCardEvaluasi = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/evaluation/get-by-company`, {
                ...
                <CCard className="mb-4" style={{ overflowX: 'scroll' }}>
                  <CCardBody>
                    <Row align="middle">
                      ...
                      <Col span={item.status_submit === 0 ? 16 : 19}>{item.participant_name}</Col>
