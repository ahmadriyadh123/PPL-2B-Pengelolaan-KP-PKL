...
const FormulirEvaluasiPerusahaan = () => {
  ...
  const columns = [
      ...
      render: (value, item, index) => (
        <>
          {index < assessment_aspect + 1 ? (
            ...
            <>
              <Button type="primary" onClick={() => addAspek()} size="small">
            ...
            <div style={{ textAlign: 'left' }}>
              <Row align="middle">
                <Col span={1} style={{ paddingRight: '10px' }}>
                  {numToSSColumn(index - assessment_aspect).toLowerCase()}.
                </Col>
                <Col span={23}>
                  <Form.Item
                    name={`aspek-${index}`}
    ...
    },
    {
      ...
      dataIndex: 'nilai',
      onCell: (record) => {
        return { colSpan: record.name === 'Kemampuan Lain' ? 0 : record.name === '' ? 0 : 1 }
      },
      render: (value, item, index) => (
        <>
          {item.name !== 'Kemampuan Lain' && (
            <Form.Item
              ...
              style={{ marginBottom: '0px' }}
              rules={[
                { required: true, message: 'Nilai tidak boleh kosong!' },
                { message: 'Nilai hanya bisa diisi oleh angka!', pattern: /^(?:\d*)$/ },
                {
                  message: 'Nilai minimal adalah 50!',
                  pattern: /\b^([A-Za-z]+|[1-9]\d{2,}|[5-9]\d)$\b/,
                },
                {
                  message: 'Nilai maksimal adalah 100!',
                  pattern: /\b([A-Za-z]+|[0-9]|[0-9][0-9]|0+|100)\b/,
                },
              ]}
            >
              <Input />
  ...
  const handleOkCreate = async (index) => {
    ...
    valuation = valuation
      .filter((item) => item.aspect_name !== 'Kemampuan Lain')
      .filter((item) => item.aspect_name !== '')
    axios.defaults.withCredentials = true
    location.state.id_prodi === 0
      ? (catatan = form.getFieldValue('catatan'))
      : parseInt(location.state.numEval) === 3
      ? (catatan = form.getFieldValue('catatanExcelent') + '|' + form.getFieldValue('catatanLemah'))
      : (catatan = form.getFieldValue('catatan'))
    await axios
      .put(`${process.env.REACT_APP_API_GATEWAY_URL}company/evaluation/update/${id}`, {
        comment: catatan,
  ...
  useEffect(() => {
    ...
    if (location.state === undefined) {
      ...
      const getFormEvaluasi = async () => {
        ...
        await axios
          .get(
            `${process.env.REACT_APP_API_GATEWAY_URL}company/evaluation/form?id=${id}&numeval=${location.state.numEval}&prodi=${location.state.id_prodi}`,
            {
          ...
          .then(function (response) {
            ...
            location.state.id_prodi === 1 &&
              parseInt(location.state.numEval) === 3 &&
              item.assessment_aspect.push('Kemampuan Lain')
            location.state.id_prodi === 1 &&
              parseInt(location.state.numEval) === 3 &&
              item.assessment_aspect.push('')
            setData(item)
        ...
        <CCardHeader style={{ paddingLeft: '20px' }}>
          <Row>
            <Col span={12} style={{ textAlign: 'left' }}>
              <h5>
                <b>Formulir Evaluasi {location.state.name}</b>
              </h5>
            </Col>
        ...
        <CCardBody style={{ paddingLeft: '20px' }}>
          <Row style={{ border: '1px solid black' }}>
            <Col span={24}>
              <Row style={{ height: '90px' }} justify="space-around" align="middle">
                ...
                <Col
                  span={20}
                  style={{ borderBottom: '1px solid black', textAlign: 'center', height: '90px' }}
                >
                  <Row
            ...
            <Col span={24}>
              <Row style={{ height: '45px' }} justify="space-around" align="middle">
                ...
                <Col
                  span={13}
                  style={{ borderRight: '1px solid black', textAlign: 'center', height: '45px' }}
                >
                  <Row style={{ height: '45px' }} justify="space-around" align="middle">
                    <Col span={24}>
                      <b>
          ...
          <Row style={{ border: '1px solid black' }}>
            <Col span={24}>
              <Row style={{ height: '90px' }}>
                ...
                <Col span={14}>
                  <Row style={{ height: '30px' }} justify="space-around" align="middle">
                    <Col span={24}>{`${data.cp_name} / ${data.cp_email} / ${data.cp_phone}`}</Col>
                  </Row>
                </Col>
              </Row>
            </Col>
          ...
          <Row justify="space-around" align="middle" style={{ paddingTop: '20px' }}>
            <Col span={24} style={{ textAlign: 'center' }}>
              <p style={{ textAlign: 'justify' }}>
                Evaluasi yang Bapak/ Ibu berikan akan sangat membantu kami dalam memperbaiki sistem
                pengajaran maupun program pada masa yang akan datang. Mohon kesediaannya untuk
                mengevaluasi peserta {location.state.id_prodi === 0 ? 'KP D3' : 'PKL D4'} Teknik
                Informatika JTK yang ada dalam lingkungan unit Bapak/ Ibu dengan memberikan
          ...
          >
            <Table
              ...
              columns={columns}
              dataSource={dataAspek}
              rowKey={(record) => record.id}
              pagination={false}
              bordered
            />
            <b>
              Catatan/Masukan:<span style={{ color: 'red' }}> * </span>
            </b>
            {location.state.id_prodi === 0 ? (
              <>
                <Form.Item
