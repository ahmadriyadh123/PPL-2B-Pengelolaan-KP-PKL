...
const DetailFeedbackPerusahaan = () => {
  ...
  useEffect(() => {
    ...
    const getDetail = async () => {
      ...
      await axios
        .get(
          `${process.env.REACT_APP_API_GATEWAY_URL}company/feedback/${id}/${location.state.id_prodi}`,
          {
          ...
          <CCardHeader style={{ paddingLeft: '20px' }}>
            {localStorage.getItem('id_role') === '0' || localStorage.getItem('id_role') === '3' ? (
              ...
              <h5>
                <b>
                  Feedback Pelaksanaan{' '}
                  {parseInt(location.state.id_prodi) === 0
                    ? 'Kerja Praktik'
                    : 'Praktik Kerja Lapangan'}
                </b>
              </h5>
              ...
              {data.map((item, index) => (
                <div key={index}>
                  ...
                  <div style={{ paddingTop: '10px' }}>
                    <Form.Item name={`pertanyaan-${item.id}`} style={{ marginBottom: '0px' }}>
                      <Row>
                        <Radio.Group value={item.value} disabled={true} style={{ width: '100%' }}>
                          <Row>
