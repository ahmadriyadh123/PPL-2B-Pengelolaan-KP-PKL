...
const ListPerusahaan = () => {
  ...
  const [status, setStatus] = useState({
    statusAktif: { check: false, x: false },
    statusPrerequisite: { check: false, x: false },
  })
  ...
  useEffect(() => {
    ...
    const getCompany = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/list`, {
          signal: controller.signal,
        })
        .then(function (response) {
          setData(response.data.data)
          axios
            .get(`${process.env.REACT_APP_API_GATEWAY_URL}company/prerequisite/recap`, {
              signal: controller.signal,
            })
            .then(function (response) {
  ...
  const columnsKompetensi = {
    MinatPekerjaan: [
      ...
      },
      {
        ...
        width: '25%',
        align: 'center',
        render: (value, item, index) => <>{getPersentase(item.total)}%</>,
      },
    ],
    BahasaPemrograman: [
      {
            ...
            <CRow>
              <CCol sm={3}>
                <CCard
                  className="mb-4"
                  id="card-filter"
                  onClick={() => changeData('statusAktifCheck')}
                  style={
                    status.statusAktif.check === true
              ...
              <CCol sm={3}>
                <CCard
                  className="mb-4"
                  id="card-filter"
                  onClick={() => changeData('statusAktifX')}
                  style={
                    status.statusAktif.x === true
                      ? { backgroundColor: '#C8C8C8' }
                      : { backgroundColor: 'white' }
                  }
                >
                  <CCardBody style={{ padding: '10px' }}>
                    <Row justify="space-around" align="middle">
                      <Col span={24} style={{ paddingTop: '10px' }}>
                        <h6 style={{ fontSize: '9pt' }}>
                          <b>Perusahaan Tidak Aktif</b>
                        </h6>
                        <h5 style={{ color: '#CC0033' }}>
                          {data.total_company_inactive} Perusahaan
                        </h5>
                      </Col>
                    </Row>
                  </CCardBody>
              ...
              <CCol sm={3}>
                <CCard
                  className="mb-4"
                  id="card-filter"
                  onClick={() => changeData('statusPrerequisiteCheck')}
                  style={
                  ...
                  <CCardBody style={{ padding: '10px' }}>
                    <Row justify="space-around" align="middle">
                      ...
                      <Col span={18} style={{ paddingTop: '10px' }}>
                        <h6 style={{ fontSize: '8pt' }}>
                          <b>Sudah Mengisi Formulir Prerequisite</b>
                        </h6>
                        <h5 style={{ color: '#339900' }}>
                          {data.total_prerequisite_submitted} Perusahaan
                        </h5>
                      </Col>
                    </Row>
                  </CCardBody>
              ...
              <CCol sm={3}>
                <CCard
                  className="mb-4"
                  id="card-filter"
                  onClick={() => changeData('statusPrerequisiteX')}
                  style={
                ...
                >
                  <CCardBody style={{ padding: '10px' }}>
                    <Row justify="space-around" align="middle">
                      ...
                      <Col span={18} style={{ paddingTop: '10px' }}>
                        <h6 style={{ fontSize: '8pt' }}>
                          <b>Belum Mengisi Formulir Prerequisite</b>
                        </h6>
                        <h5 style={{ color: '#CC0033' }}>
                          {data.total_prerequisite_notsubmitted} Perusahaan
                        </h5>
                      </Col>
                    </Row>
                  </CCardBody>
                      ...
                      dataSource={
                        status.statusAktif.check === false
                          ? status.statusAktif.x === false
                            ...
                            ? data.company.filter(
                                (item) =>
                                  item.status_company === false &&
                                  item.status_prerequisite === true,
                              )
                            : status.statusPrerequisite.x === true
                            ? data.company.filter(
                                (item) =>
                                  item.status_company === false &&
                                  item.status_prerequisite === false,
                              )
                            : data.company.filter((item) => item.status_company === false)
                          : status.statusPrerequisite.check === true
                          ? data.company.filter(
                              (item) =>
                                item.status_company === true && item.status_prerequisite === true,
                            )
                          : status.statusPrerequisite.x === true
                          ? data.company.filter(
                              (item) =>
                                item.status_company === true && item.status_prerequisite === false,
                            )
