...
const FormulirFeedbackPerusahaan = () => {
  ...
  const handleOkCreate = async (index) => {
    ...
    await axios
      .put(
        `${process.env.REACT_APP_API_GATEWAY_URL}company/feedback/update/${location.state.id_prodi}`,
        jawaban,
      )
      .then(function (response) {
        notification.success({
          message: `Feedback pelaksanaan ${
            location.state.id_prodi === 0 ? 'kerja praktik' : 'praktik kerja lapangan'
          } berhasil diisi`,
        })
        setLoadings((prevLoadings) => {
          const newLoadings = [...prevLoadings]
  ...
  useEffect(() => {
    ...
    const getDetail = async () => {
      ...
      await axios
        .get(
          `${process.env.REACT_APP_API_GATEWAY_URL}management-content/feedback-question/get-all/${location.state.id_prodi}`,
          {
                    ...
                    <Radio.Group style={{ width: '100%' }}>
                      <Row>
                        <Col style={{ textAlign: 'center', width: '20%' }}>
                          <Radio value={5}>Sangat baik</Radio>
                          <br />
                        </Col>
                        <Col style={{ textAlign: 'center', width: '20%' }}>
                          <Radio value={4}>Baik</Radio>
                          <br />
                        </Col>
                        <Col style={{ textAlign: 'center', width: '20%' }}>
                          <Radio value={3}>Cukup baik</Radio>
                          <br />
                        </Col>
                        <Col style={{ textAlign: 'center', width: '20%' }}>
                          <Radio value={2}>Kurang baik</Radio>
                          <br />
                        </Col>
                        <Col style={{ textAlign: 'center', width: '20%' }}>
