...
const DataMahasiswa = () => {
  ...
  useEffect(() => {
    ...
    const getCV = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/cv-by-company`, {
  ...
  const exportPDF = async (item, index) => {
    ...
    axios.defaults.withCredentials = true
    await axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/cv/${item.id_cv}/export`, {
        responseType: 'blob',
      })
      .then((response) => {
              ...
              <CCardBody>
                <Row align="middle">
                  ...
                  <Col span={3} style={{ textAlign: 'center' }}>
                    ...
                    >
                      <FontAwesomeIcon icon={faFileDownload} style={{ paddingRight: '5px' }} />{' '}
                      Export
                    </Button>
                  </Col>
                </Row>
          ...
          <CCard>
            <CCardBody>
              <Row align="middle" justify="space-around">
                <Col>
                  <h3 style={{ paddingTop: '10px' }}>Pemetaan Belum dilakukan</h3>
                </Col>
              </Row>
            </CCardBody>
          </CCard>
