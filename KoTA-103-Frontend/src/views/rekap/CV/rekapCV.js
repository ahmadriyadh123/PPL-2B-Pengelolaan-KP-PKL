...
const RekapCV = () => {
  ...
  useEffect(() => {
    ...
    const getRecap = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/cv/recap`, {
    ...
    const getCV = async () => {
      axios.defaults.withCredentials = true
      await axios
        .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/cv-by-committee`, {
          signal: controller.signal,
        })
        .then(function (response) {
  ...
  const exportPDF = async (record, index) => {
    ...
    axios.defaults.withCredentials = true
    await axios
      .get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/cv/${record.id_cv}/export`, {
        responseType: 'blob',
      })
      .then((response) => {
          ...
          <Row>
            ...
            <Col span={11} style={{ textAlign: 'left' }}>
              <Button
                ...
                onClick={() => {
                  exportPDF(record, `cv-${record.id_cv}`)
                }}
              >
  ...
  const getPersentase = (total) => {
    return localStorage.getItem('id_role') === '3'
      ...
      : ((total / data.cv.length) * 100).toFixed(2)
  }
                        ...
                        <Row justify="space-around" align="middle">
                          ...
                          <Col span={18} style={{ paddingTop: '10px' }}>
                            ...
                            <h5 style={{ color: '#CC0033' }}>
                              {data.total_not_submitted} Mahasiswa
                            </h5>
                          </Col>
                        </Row>
                      </CCardBody>
