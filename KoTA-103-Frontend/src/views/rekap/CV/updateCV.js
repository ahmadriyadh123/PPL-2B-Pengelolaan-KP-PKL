...
const UpdateCV = () => {
    ...
    useEffect(() => {
        const getCV = async () => {
            ...
            axios.defaults.withCredentials = true;
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}participant/cv/detail/${id}`);
                ...
                const domicileResponse = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/domicile/${response.data.data.domicile_id}`)
                setRegion(domicileResponse.data.data)

                const jobscopeResponse = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/jobscope/get-all/${parseInt(localStorage.getItem("id_role")) !== 2 ? parseInt(localStorage.getItem("id_prodi")) : 0}`)
                setJobscope(jobscopeResponse.data.data)

                const kompetensiResponse = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/competence/get-all/${parseInt(localStorage.getItem("id_role")) !== 2 ? parseInt(localStorage.getItem("id_prodi")) : 0}`)
                setKompetensi(kompetensiResponse.data.data)

                const tipeResponse = await axios.get(`${process.env.REACT_APP_API_GATEWAY_URL}management-content/competence/get-all-type`)
